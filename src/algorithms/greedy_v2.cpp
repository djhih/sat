// aim to set more receivers and more transmitters
...
double alpha;
int transmitter_num = 1; // 每個衛星的發射器數量
int receiver_num = 1; // gs 
...
// add map<int, int> to store the transmitters usage of each satellite
// add map<int, int> to store the receivers usage of each ground station
...
void data_process(){
    ...
    for(int i=0; i<S; i++){
        trans_cnt[i] = transmitter_num;
    }
    for(int i=0; i<G; i++){
        rec_cnt[i] = receiver_num;
    }
...
int main(int argc, char* argv[]){
    if(argc == 5){
        transmitter_num = stoi(argv[1]);
        receiver_num = stoi(argv[2]);
        infile = argv[3];
        outfile = argv[4];
    }
    input();
